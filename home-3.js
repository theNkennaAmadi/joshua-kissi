...
mm.add("(min-width: 480px)", () => {
  ...
  gsap.utils.toArray(".section.hero").forEach((panel, i) => {
    ScrollTrigger.create({
      ...
      onEnter: () => {
        //goToSection(i);
        if (i > 0) {
          let allPrevious = indexNodes[i].previousElementSibling;
          let tl = gsap.timeline();
          tl.to(allPrevious, { y: "-110%", ease: "expo.inOut" });
          tl.to(indexNodes[i], { y: "0%", ease: "expo.inOut" });
        }

        /*
      let text = panel.querySelector(".m-img").style.backgroundImage;
      let fIndex = text.lastIndexOf("rgb")
      let lIndex = text.lastIndexOf("100%")
      let endColor = text.substring(fIndex,lIndex).trimEnd()
      bgImage.style.setProperty('--boxAfterBackColor', endColor);
      */
        bgImageAfter.style.backgroundImage = panel.querySelector(
          ".hp-section-bg"
        ).style.backgroundImage;
        bgImageAfter.style.opacity = 1;
        bgImage.style.backgroundImage = panel.querySelector(
          ".hp-section-bg"
        ).style.backgroundImage;
    ...
    ScrollTrigger.create({
      ...
      start: "bottom bottom",
      onEnterBack: () => {
        let allPrevious = indexNodes[i].nextElementSibling;
        let tl = gsap.timeline();
        tl.to(allPrevious, { y: "110%", ease: "expo.inOut" });
        tl.to(indexNodes[i], { y: "0%", ease: "expo.inOut" });
  ...
  ScrollTrigger.create({
    ...
    onLeave: () => {
      gsap.to(".index-main-wrapper", { opacity: 1 });
    }
  });
});
...
navs.map((nav) => {
  ...
  nav.addEventListener("click", () => {
    let index = navs.indexOf(nav);
    console.log(index);
    let target = allItems[index].getBoundingClientRect().top;
    gsap.to(window, { duration: 0, scrollTo: target });
    gsap.to(window, { duration: 0, scrollTo: target });
    //goToSection(allItems[index]);
